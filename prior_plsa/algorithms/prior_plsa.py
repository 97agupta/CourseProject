...
class PriorPLSA(BasePLSA):
    ...
    """
    def __init__(self, corpus: Corpus, n_topics: int, tf_idf: bool = True, mu: float = 0, prior_topic: ndarray = None):
        super().__init__(corpus, n_topics, tf_idf)
        self._word_given_topic = empty((corpus.n_words, self.n_topics))
        self.__mu = mu
        self.__prior_topic = prior_topic

    def _m_step(self) -> None:
        """Implements the M-step of EM-style algorithm to train PLSA model."""
        self._doc_given_topic = self._norm_sum('dw,tdw->dt')

        if (self.__prior_topic is None) or (self.__mu == 0):
            self._word_given_topic = self._norm_sum('dw,tdw->wt')
        else:
            probability = einsum('dw,tdw->wt', self._doc_word, self._conditional)

            probability[probability < MACHINE_PRECISION] = 0.0
            norm = probability.sum(axis=0)
            norm[norm == 0.0] = 1.0

            num = (self.__prior_topic * self.__mu)+probability
            denom = norm + self.__mu
            self._word_given_topic = num/denom
